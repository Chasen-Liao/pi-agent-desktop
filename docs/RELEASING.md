...
4. 从合并后的 `main` 创建干净 worktree并执行 `npm ci`：
   ...
   - Linux 运行 `npm run dist`，生成 DEB 安装包。Linux 打包机需要 `fakeroot` 与 `dpkg`（提供 `dpkg-deb`）；Debian/Ubuntu 自带，Arch 需另装 `dpkg` 与 `libxcrypt-compat`（fpm 自带的 Ruby 需要 `libcrypt.so.1`）。
